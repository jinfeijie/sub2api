export default {
    accounts: {
      ...
      usageWindowsHint: '“5h / 7d”等为上游滚动用量窗口（OpenAI ChatGPT、Claude 等）。Grok 订阅为周限（滚动约 7 天）与月额积分（自然月账期，按 billing 起止日重置，不是滚动 30 天）。均由上游对账号设定，并非 sub2api 配置，也与你映射的模型无关。窗口/账期到期后用量会自动重置，无法在 sub2api 端解除该限制。',
      ...
      usageWindow: {
        ...
        grokRequests: '请求',
        grokTokens: 'Token',
        grokUnknown: 'Grok 额度需点击「查询」读取订阅账单，或等待上游响应返回 rate-limit 头。',
        grokRetryAfter: '{time} 后重试',
        grokProbeTooltip: '读取 xAI CLI 账单接口（周限额 / 月度积分）',
        grokProbeErrorReauth: '授权失效，请重新授权',
        grokProbeErrorEmpty: '未读到额度数据',
        grokProbeErrorRateLimit: '上游限流，请稍后重试',
        grokProbeErrorNetwork: '网络异常，请稍后重试',
        grokProbeErrorUpstream: '上游查询失败',
        grokProbeErrorGeneric: '查询失败',
        grokProbePersistWarning: '额度已获取，但暂未保存',
        grokResetUnsupported: '不支持重置',
        grokResetUnsupportedTooltip: 'xAI 未向 Grok OAuth 账号开放重置额度接口',
        grokNoHeaders: '未观察到配额数据',
        grokApiUsage: 'Api',
        grokMonthlyCreditsLabel: '月额',
        grokMonthlyBarLabel: '月',
        passiveSampled: '被动采样',
        activeQuery: '查询'
      },
      openaiQuotaReset: {
