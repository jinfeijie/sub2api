...
func TestHandleGrokAccountUpstreamErrorLeavesWindowComparisonToRepository(t *testing.T) {
	...
	require.Equal(t, 1, repo.tempUnschedCalls)
	require.Less(t, repo.lastTempUnschedUntil, existingUntil)
	...
	require.Equal(t, "rate_limit", state.MatchedKeyword)
}
